...
import { ArrowLeft, Search, ShoppingCart, Plus, Minus, X, Share2, AlertTriangle } from 'lucide-react';
...
export function StoreDetailClient({
  ...
  const hasItemsFromOtherStore = items.length > 0 && items.some((i) => i.storeId !== store.id);
      ...
      <header className="relative w-full h-48 sm:h-56 md:h-64 lg:h-80 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 md:bottom-6 md:left-6 lg:bottom-8 lg:left-8 z-20 flex items-end gap-3 sm:gap-4">
          ...
          <div className="mb-0.5 text-white flex-1 min-w-0">
            <h2 className="text-lg sm:text-xl md:text-2xl lg:text-4xl font-bold leading-tight truncate">{store.name}</h2>
            {store.category && (
              <p className="text-xs sm:text-sm md:text-lg text-white/80 font-medium truncate">{store.category.name}</p>
            )}
          </div>
        </div>
      </header>
