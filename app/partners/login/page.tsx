...
export default function PartnerLoginPage() {
  ...
  const validateRegister = () => {
    const newErrors: Record<string, string> = {}
    if (!registerData.businessName) newErrors.businessName = "El nombre del negocio es requerido"
    else if (registerData.businessName.length < 3) {
      newErrors.businessName = "Mínimo 3 caracteres"
    }
    if (!registerData.phone) newErrors.phone = "El teléfono es requerido"
    else if (!/^\+?[\d\s-]{8,}$/.test(registerData.phone.replace(/\s/g, ""))) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const response = await authApi.partners.login({
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const response = await authApi.partners.register({
      ...
      setActiveTab("login")
      setRegisterData({ businessName: "", phone: "", email: "", password: "", confirmPassword: "" })
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-4">
      ...
      <div className="relative w-full max-w-md">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Portal de Aliados</p>
          </div>
        </div>
          ...
          <Card className="border-0 shadow-2xl shadow-slate-200/50 dark:shadow-slate-900/50">
            <CardHeader className="space-y-1 pb-4">
              <CardTitle className="text-2xl font-bold text-center">
                {activeTab === "login" ? "Bienvenido de nuevo" : "Únete como aliado"}
              </CardTitle>
              <CardDescription className="text-center">
                {activeTab === "login"
                  ? "Ingresa tus credenciales para acceder al panel"
                  : "Registra tu negocio para comenzar a vender"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <TabsContent value="register" className="space-y-4">
                <form onSubmit={handleRegister} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="register-businessName">Nombre del negocio</Label>
                    <Input
